  ...
  renderPromptCard(prompt) {
    return `
      <div class="prompt-card animate-in" data-prompt-id="${prompt.id}">
        ...
        <a href="index.html?prompt=${prompt.id}" class="prompt-link">詳細を見る</a>
      </div>
    `;
  }
  ...
  renderMenuItem(prompt) {
    return `
      <li class="prompt-library-menu-item" data-prompt-id="${prompt.id}">
        <a href="index.html?prompt=${prompt.id}" class="prompt-library-menu-link">
          <i class="${prompt.icon}"></i>
  ...
  setupEventListeners() {
    ...
    promptCards.forEach(card => {
      card.addEventListener('click', (e) => {
        ...
        window.location.href = `index.html?prompt=${promptId}`;
      });
    });
