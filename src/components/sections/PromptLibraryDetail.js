class PromptLibraryDetail extends HTMLElement {
  ...
  async loadPrompts() {
    try {
      ...
      this.showError('プロンプトの読み込みに失敗しました');
    }
  }
  ...
  loadPromptsViaJSONP() {
    return new Promise((resolve, reject) => {
      const callbackName = 'handlePromptsData_' + Date.now();
      ...
      script.src = `${this.apiUrl}?action=list&callback=${callbackName}`;
      script.onerror = () => {
        delete window[callbackName];
        reject(new Error('Failed to load prompts via JSONP'));
      };
  ...
  // 特定のプロンプト詳細を取得
  async loadPromptDetail(promptId) {
    try {
      // IDを文字列に確実に変換
      ...
      if (isLocalOrPreview) {
        ...
        // 本番環境では通常のfetch
        const response = await fetch(`${this.apiUrl}?action=detail&id=${idStr}`);
        const data = await response.json();
        
        if (data.prompt) {
          this.displayPromptDetail(data.prompt);
        } else {
          this.showError('プロンプトが見つかりませんでした');
        }
      }
    } catch (error) {
      console.error('Error loading prompt detail:', error);
      this.showError('プロンプトの詳細読み込みに失敗しました');
    }
  }

  // JSONP経由でプロンプト詳細を読み込む
  loadPromptDetailViaJSONP(promptId) {
    return new Promise((resolve, reject) => {
      const callbackName = 'handlePromptDetail_' + Date.now();
      ...
      const script = document.createElement('script');
      script.src = `${this.apiUrl}?action=detail&id=${promptId}&callback=${callbackName}`;
      script.onerror = () => {
        delete window[callbackName];
        reject(new Error('Failed to load prompt detail via JSONP'));
      };
      document.head.appendChild(script);
    });
  }
